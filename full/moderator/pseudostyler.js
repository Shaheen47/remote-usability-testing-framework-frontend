class PseudoStyler {
  ...
  async loadDocumentStyles() {
    let count = this.doc.styleSheets.length;
    for (let i = 0; i < count; i++) {
      let sheet = this.doc.styleSheets[i];
      if (sheet.href) {
  ...
  addCSS(text) {
    let copySheet = this.doc.createElement('style');
    copySheet.type = 'text/css';
    copySheet.textContent = text;
    this.doc.head.appendChild(copySheet);
    ...
    this.doc.head.removeChild(copySheet);
  }
